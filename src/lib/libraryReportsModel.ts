...
import { GLOSSARY_SET_DEFS, glossarySetById } from './glossarySets';
import { normalizeGlossaryRecordSetKey } from './crossSetTargetPreview';
...
function glossarySetLabelForKey(setKey: string): string {
  if (!setKey) return GLOSSARY_SET_UNASSIGNED_LABEL;
  const def = glossarySetById(setKey);
  return def?.name ?? setKey;
}

function hasMissingGlossarySetMetadata(entity: { fldGlossarySetId?: string } | undefined): boolean {
  return !String(entity?.fldGlossarySetId ?? '').trim();
}
...
export function buildStandardsAssociationsReport(input: {
  ...
  for (const [setKey, nodes] of setMap.entries()) {
    nodes.sort((a, b) => a.citationLabel.localeCompare(b.citationLabel, undefined, { sensitivity: 'base' }));
...
export function buildGlossaryHierarchyReport(input: {
  ...
  type RowAcc = {
    ...
    rowMissingSet: boolean;
  };

  const rows: RowAcc[] = [];
  for (const g of glossary) {
    if (g.fldDeleted || g.fldIsDeleted) continue;
    const setKey = normalizeGlossaryRecordSetKey(g.fldGlossarySetId);
    rows.push({
      setKey,
      ...
      rowMissingSet: !String(g.fldGlossarySetId ?? '').trim(),
    });
  }
  ...
  for (const setKey of orderedSetKeys) {
        ...
        for (const [findId, recMap] of findMap.entries()) {
          ...
          for (const [recId, accRows] of recMap.entries()) {
            ...
            findingNode.recommendations.push({
              ...
              missingGlossarySetMetadata:
                accRows.some((r) => r.rowMissingSet) ||
                hasMissingGlossarySetMetadata(fEntity) ||
                hasMissingGlossarySetMetadata(rEntity),
            });
    ...
    groups.push({
      setKey,
      setLabel: glossarySetLabelForKey(setKey),
