...
**Status:** Documentation-only (D1) for mapping/extraction. **Project metadata persistence** for `tdlrRegistered` / RAS assignments is implemented on `feat/ras-beta-project-metadata` (New/Edit Project only). RAS report **covers** and **body** are implemented (`feat/ras-report-cover`, `feat/ras-report-body`).
**Last updated:** 2026-08-29 (report Slice C: RAS body)
**Branch context:** `feat/ras-report-body`
