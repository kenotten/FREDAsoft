...
✅ DECIDED (Data Entry explicit glossary citation snapshot refresh): Saved `projectData.fldStandards` remains a **snapshot**. **Opening** an existing record does **not** rehydrate citations from current glossary or master data (empty `[]` stays `[]`). Assessment: explicit glossary recommendation selection refreshes the **unsaved working** list (`standardsIdsFromGlossaryRow` on new records; selected row `fldStandards` on existing records). Assessment Finding selection alone does not hydrate citations. TAS/RAS: selecting or changing a Finding hydrates working citations immediately via finding-first `findingCitationIdsFromGlossaryRow` from the **internally resolved** glossary row (prefer current `glosId` / `fldData` when that row still matches the selected cat/item/find; else first path match — same `rasFindingPathCitationState` resolver as **Refresh TAS References**). Recommendation selection is not required. Multiple glossary rows that differ only by Recommendation do not block hydration and do not invent rec text, cost, or rec provenance. Glossary document identity is not a user-facing duplicate choice. Rec text and cost remain hidden on RAS. An explicit **Refresh TAS References** action recalculates working citations the same way. Hydrated or refreshed IDs persist only if the user **Saves**. No automatic historical backfill. No live glossary/finding union at report time.
